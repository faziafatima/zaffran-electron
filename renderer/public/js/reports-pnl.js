...
function renderSalesRows(rowsId, rows) {
  ...
  tbody.innerHTML = rows.map(row => `
    <tr>
      ...
      <td>${formatMoney(row.netProfit || 0)}</td>
    </tr>
  `).join('');
}
...
function buildCsvContent(period, report) {
  ...
  const csvLines = [
    'PNL Report',
    ['Month', 'Sales', 'Expenses', 'Net Profit'].map(escapeCsvCell).join(',')
  ];
...
function exportPeriodCsv(period) {
  ...
  const filename = `pnl-report-${period}-${selectedDate}.csv`;
...
console.log('Server context:', headerRestaurantId);
  try {
    const response = await fetch(`/api/reports/profit-loss/${headerRestaurantId}?dateFrom=${encodeURIComponent(selectedDate)}&dateTo=${encodeURIComponent(selectedDateTo)}`);
    if (!response.ok) {
      throw new Error(`Unable to load reports. Status ${response.status}`);
    }

    const data = await response.json();
    renderPeriod('pnl', data || {});
...
function initializeReportsPage() {
  ...
  const pnlExportButton = document.getElementById('pnlExportCsvButton');
  ...
  if (pnlExportButton) {
    pnlExportButton.addEventListener('click', () => exportPeriodCsv('pnl'));
  }
