...
function renderSalesRows(rowsId, rows) {
  ...
  tbody.innerHTML = rows.map(row => `
    <tr>
      <td>${row.itemName || '-'}</td>
      <td>${row.unit || '-'}</td>
      <td>${Number(row.currentStock || 0)}</td>
      <td>${Number(row.dailyUsage || 0)}</td>
    </tr>
  `).join('');
}
...
function buildCsvContent(period, report) {
  ...
  const csvLines = [
    'Inventory Report',
    ['Item Name', 'Unit', 'Current Stock', 'Daily Usage'].map(escapeCsvCell).join(',')
  ];
...
function exportPeriodCsv(period) {
  ...
  const filename = `inventory-report-${period}-${selectedDate}.csv`;
...
console.log('Server context:', headerRestaurantId);
  try {
    const response = await fetch(`/api/reports/inventory/${headerRestaurantId}?dateFrom=${encodeURIComponent(selectedDate)}&dateTo=${encodeURIComponent(selectedDateTo)}`);
    if (!response.ok) {
      throw new Error(`Unable to load reports. Status ${response.status}`);
    }

    const data = await response.json();
    renderPeriod('inventory', data || {});
...
function initializeReportsPage() {
  ...
  const inventoryExportButton = document.getElementById('inventoryExportCsvButton');
  ...
  if (inventoryExportButton) {
    inventoryExportButton.addEventListener('click', () => exportPeriodCsv('inventory'));
  }
